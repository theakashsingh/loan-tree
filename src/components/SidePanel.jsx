...
import { getAllowedChildren, NODE_COLORS, NODE_ICONS, NODE_TYPES } from '../constants/nodeTypes';

const SidePanel = ({ node, onAddChild, onDeleteNode, onClose }) => {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center mb-3">
            <div className={`w-10 h-10 rounded-full ${NODE_COLORS[node.type]} flex items-center justify-center mr-3`}>
          ...
          <div className="text-sm text-gray-600">
            {node.type === NODE_TYPES.ACCOUNT && "Represents a customer's account"}
            {node.type === NODE_TYPES.LOAN && "A loan issued to an account"}
            {node.type === NODE_TYPES.COLLATERAL && "Asset pledged against a loan"}
          </div>
            ...
            <div className="space-y-2">
              {allowedChildren.map(childType => {
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full ${NODE_COLORS[childType]} flex items-center justify-center mr-3`}>
                      <ChildIcon className="w-4 h-4 text-white" />
                    </div>
                    <span className="font-medium text-gray-700">Add {childType}</span>
